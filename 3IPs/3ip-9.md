...
**Joining a chat:**
```js
const thread = await space.joinThread('name-of-thread', { ghost: true }) 
...
const userList = await thread.getPosts()
```
...
- `backlog_response` for when a peer fulfills a backlog request
- `backlog` for when a user receives a backlog from other peers
